/* Author: Yash yash@speedovation.com Owner: Diode
 *
 * AIM : Update exchanges last prices for DIO market when requested
 *
 * Overview screen will call refresh and connected using signal.
 *
 * On the overview page we are pulling current prices from the markets to show the current price for DIO.
 *
 */
...
/*
Mintpal
https://api.mintpal.com/v2/market/stats/DIO/BTC
we want to pull the info from “last_price”
https://api.mintpal.com/v2/market/stats/AUR/BTC is an example as the market DIO has not been created.
*/
...
https://bittrex.com/api/v1.1/public/getmarketsummary?market=btc-aur is an example as the market DIO has not been created.
*/
...
//    market = "SUPER";
    ...
    QUrl url( QString("https://bittrex.com/api/v1.1/public/getmarketsummary?market=btc-dio").arg(market) );
    QNetworkReply* reply = nam->get(QNetworkRequest(url));
...
https://c-cex.com/t/btc-usd.json is an example as the market DIO has not been created.
*/
...
//    market = "usd";
    ...
    QUrl url( QString("https://c-cex.com/t/dio-btc.json").arg(market) );
    QNetworkReply* reply = nam->get(QNetworkRequest(url));
...
we need to pull info from line BTC_DIO
...
//    market = "AUR";
    ...
    float r = jsonObject[QString("BTC_DIO").arg(market)].toObject().toVariantMap().value("last").toFloat();
...
void HashMarketApi::cryptsyFinished()
{
    ...
    QString str = jsonObject["return"].toObject()["markets"].toObject()["DIO"].toObject().toVariantMap().value("lasttradeprice").toString();
